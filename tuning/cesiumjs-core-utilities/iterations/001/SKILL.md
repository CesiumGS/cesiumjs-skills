...
> **This is a breaking change.** Any code using `defaultValue(...)` will throw
> a `ReferenceError` in CesiumJS v1.134+. Replace ALL occurrences with `??`.
...
> **Always wrap Resource fetches in try/catch.** All `fetch*` methods throw
> on HTTP errors (404, 500) and network failures. Unhandled rejections crash
> the application silently in production.
...
> **Named Color constants are frozen.** `Color.RED.alpha = 0.5` silently
> fails (or throws in strict mode). Use `Color.RED.withAlpha(0.5)` to get
> a new Color, or `Color.clone(Color.RED)` to get a mutable copy.
...
> **SPA cleanup:** In React/Angular/Vue, call `helper.removeAll()` in the
> component's unmount/destroy lifecycle hook. Leaked event listeners cause
> stale callbacks and memory growth — the same issue as undestroyed
> `ScreenSpaceEventHandler` instances.
...
> **`defined()` vs truthiness:** `defined(value)` returns `false` only for
> `null` and `undefined`. A truthiness check (`if (value)`) also rejects
> `0`, `""`, and `false` — which are valid values for heights, labels, and
> boolean properties. Always use `defined()` when checking CesiumJS properties.
