...
## Viewer Setup for Interaction Demos

When building an interaction demo, create a clean viewer with distracting widgets
disabled and position the camera immediately so the scene is ready for picking.
...
// Position camera immediately (setView, not flyTo -- no animation delay)
viewer.camera.setView({
  destination: Cartesian3.fromDegrees(-75.59, 40.04, 15000),
  orientation: {
    heading: 0.0,
...
> **SPA cleanup:** In React, Angular, or Vue apps, destroy the handler in
> the component's cleanup/unmount lifecycle hook. Each undestroyed handler
> registers DOM listeners that leak memory and fire ghost events. This is
> the #1 CesiumJS support issue in SPA frameworks.
...
// drillPick -- all objects at position, front-to-back; ALWAYS pass a limit
const allPicked = viewer.scene.drillPick(event.position, 5); // limit to 5
...
> **drillPick without a `limit`** re-renders the entire scene for every
> overlapping object at that pixel. Always pass a limit (e.g., `5` or `10`)
> to cap cost.

> **Picking terrain heights:** `camera.pickEllipsoid()` returns positions on
> the WGS84 ellipsoid, ignoring terrain — heights will be 0 even over mountains.
> For accurate terrain heights, use `scene.pickPosition()` with
> `globe.depthTestAgainstTerrain = true`. When both terrain and 3D Tiles are
> present, `pickPosition` with depth testing gives the most accurate results.
...
handler.setInputAction((movement) => {
  const all = viewer.scene.drillPick(movement.endPosition, 10); // always pass limit
...
> **Always restore the previous color BEFORE highlighting a new feature.**
> If you skip the restore step, the original color is lost and the feature
> stays highlighted permanently.
...
handler.setInputAction((movement) => {
  // Step 1: restore previous highlight FIRST
  if (defined(highlighted.feature)) {
    highlighted.feature.color = highlighted.originalColor;
    highlighted.feature = undefined;
  }
  // Step 2: highlight new feature
