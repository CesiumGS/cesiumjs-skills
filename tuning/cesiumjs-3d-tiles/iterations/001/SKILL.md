...
When creating a viewer for tileset work, disable unneeded widgets so the
3D content is the visual focus. Use `camera.setView` (not `flyTo`) when you
need the camera in position before a screenshot or next step -- `flyTo`
animates and may not finish in time.

```js
import {
  Viewer, Cesium3DTileset, HeadingPitchRange,
  Math as CesiumMath, Cartesian3,
} from "cesium";

// Clean viewer -- disable widgets that obscure 3D Tiles content
...
> **Camera after `zoomTo`:** `zoomTo` computes a view from the tileset's
> bounding sphere, but for city-scale tilesets you often want a specific
> perspective. After `zoomTo` completes, call `camera.setView` to set an
> explicit altitude/pitch/heading. See the altitude table below.

### Camera Positioning for Tilesets

| View type | Altitude (m) | Pitch (deg) | When to use |
|---|---|---|---|
| **Landmark / single building** | 500 -- 1,500 | -25 to -35 | One structure fills the frame |
| **City panoramic / skyline** | 800 -- 1,500 | -10 to -20 | Skyline across a river or bay. **Requires OSM Buildings or 3D Tiles.** |
| **City overview** | 2,000 -- 5,000 | -35 to -50 | Urban grid, rivers, parks visible |
| **Metro / regional** | 8,000 -- 20,000 | -60 to -90 | Entire metro area |
| **Country / continent** | 500,000 -- 5,000,000 | -90 | Large-scale context |

> **City views need 3D buildings.** For skyline, street-level, or urban views,
> add `Cesium.createOsmBuildingsAsync()` (or Google Photorealistic 3D Tiles).
> Without 3D Tiles, cities render as flat satellite imagery -- no buildings,
...
```js
// OSM Buildings -- use for any city/urban scene to get 3D silhouettes
...
> **Warning: Use regular strings for style expressions, not template literals.**
> Style expressions use `${PropertyName}` syntax which looks like JavaScript
> template literal interpolation. If you write style conditions inside
> backtick strings (`` ` ``), JavaScript will substitute variables instead of
> passing the expression to the styling engine. Always use single or double
> quotes for condition strings:
> ```js
> // WRONG -- backticks cause JS substitution, PropertyName becomes undefined
> // [`${Height} >= 100`, `color('red')`]
>
> // CORRECT -- regular strings pass the expression through to CesiumJS
> ["${Height} >= 100", "color('red')"]
> ```
...
> **Warning: `maximumScreenSpaceError` below 4 causes exponential tile growth.**
> Each halving of SSE roughly quadruples the number of tile requests. At SSE 2,
> a tileset that loads 200 tiles at SSE 16 may request 3,000+. Keep this value
> at 8+ for interactive use; use 4 only for final high-quality renders.

> **Warning: `enableCollision` interacts with globe collision detection.**
> Setting `enableCollision: true` on a tileset prevents the camera from passing
> through tileset surfaces. But if you also need the camera to stay above
> terrain, you must **also** keep `viewer.scene.screenSpaceCameraController
> .enableCollisionDetection = true` (the default). Disabling globe collision
> while tileset collision is enabled lets the camera clip through terrain
> between buildings.
