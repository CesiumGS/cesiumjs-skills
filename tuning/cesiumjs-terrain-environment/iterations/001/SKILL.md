...
```js
import { Viewer, Terrain, Cartesian3, Math as CesiumMath } from "cesium";
...
// Position camera where terrain features are visible
viewer.camera.setView({
  destination: Cartesian3.fromDegrees(-112.4, 36.06, 6000.0), // Grand Canyon
  orientation: {
...
### Camera Heights for Terrain Views

Use `camera.setView` (not `flyTo`) for immediate positioning. Choose altitude and pitch
to match the scale of terrain features:

| View type | Altitude (m) | Pitch (deg) | Notes |
|---|---|---|---|
| **Valley / canyon floor** | 300 -- 1,500 | -15 to -25 | Low altitude with moderate pitch reveals vertical relief |
| **Mountain peak** | 3,000 -- 8,000 | -25 to -40 | Shows summit and surrounding ridges |
| **Mountain range** | 15,000 -- 50,000 | -35 to -55 | Full range visible with depth |
| **Regional terrain** | 100,000 -- 500,000 | -50 to -70 | Plateaus, basins, fault lines |
| **Continental** | 2,000,000 -- 8,000,000 | -90 | Major mountain chains, rift valleys |
...
> **Gotcha: `enableLighting` is a prerequisite.** `SunLight` and `DirectionalLight`
> have no visible effect on terrain unless `globe.enableLighting = true`. Set it
> before configuring any scene lighting. Without it, the globe uses flat uniform
> illumination regardless of light source settings.
...
> **Gotcha: `depthTestAgainstTerrain` trade-off.** When `false` (default), entities
> like billboards and labels always render on top of terrain -- they float above
> hills and mountains, which looks wrong for ground-level views. When `true`,
> entities behind terrain are correctly hidden, but billboards and labels near the
> surface can disappear behind small hills or terrain tiles. **Rule of thumb:**
> set `true` for ground-clamped entities (buildings, vehicles) and when using 3D
> Tiles; keep `false` for map-style pin markers and labels that must always be visible.

> **Gotcha: Terrain + 3D Tiles interaction.** When combining terrain with 3D Tiles
> buildings (e.g., `createOsmBuildingsAsync()`), set `depthTestAgainstTerrain = true`.
> Without it, buildings appear to float above the terrain surface because the depth
> buffer does not include terrain geometry. This is essential for correct visual
> ordering of buildings against hillsides and varying elevation.
...
> **Gotcha: `verticalExaggeration` is on Scene, not Globe.**
> `globe.verticalExaggeration` does not exist and will silently do nothing.
> Always use `viewer.scene.verticalExaggeration`.
...
> **Prerequisite:** `globe.enableLighting = true` must be set for any light source
> to visually affect terrain and the globe surface. See Globe Configuration above.
...
10. **Keep `depthTestAgainstTerrain = false`** (default) unless using ground-clamped
    entities or 3D Tiles buildings -- see depthTestAgainstTerrain gotcha above.
