...
> **Cartographic uses RADIANS, not degrees.** `Cartographic.fromRadians(-105, 40)`
> treats 105 and 40 as radian values (not degrees) and silently produces a
> position in the wrong hemisphere — no error is thrown. Always use
> `Cartographic.fromDegrees(-105, 40)` or convert first with
> `CesiumMath.toRadians()`. This is the most common spatial math bug.

> **Result parameters:** Most static methods accept an optional `result`
> parameter as the last argument. In render loops, always pass a pre-allocated
> scratch object to avoid GC pressure. Some operations (e.g., `Matrix4.multiply`)
> REQUIRE a result parameter — omitting it throws.
...
> **`Cartesian3.normalize` throws on zero-length vectors.** Always check
> `Cartesian3.magnitude(v) > 0` before normalizing, especially when the
> vector comes from a subtraction that might produce (0,0,0).
...
> **Heading convention (compass bearing):** Heading follows the compass
> convention — 0 = north, increasing clockwise. This is the OPPOSITE of the
> math convention (0 = east, counterclockwise).
>
> | Heading (deg) | Heading (rad) | Camera faces |
> |---|---|---|
> | 0 | 0 | North |
> | 90 | PI/2 | East |
> | 180 | PI | South |
...
> **Column-major vs row-major:** Internal storage is column-major (WebGL).
> The `new Matrix4(...)` constructor accepts 16 numbers in ROW-major order
> for readability. Do NOT index elements assuming row-major layout — use
> `Matrix4.getColumn()` or `Matrix4.getTranslation()` instead.
