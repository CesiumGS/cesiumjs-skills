...
**Always set `Ion.defaultAccessToken` before creating a Viewer.** The token authorizes access to Cesium Ion assets (terrain, imagery, 3D Tiles). Creating a Viewer without it causes auth failures.
...
**Key API names** (regardless of import style):
- `Ion.defaultAccessToken` -- set before any Cesium calls
- `new Viewer(container, options)` -- primary entry point
- `Terrain.fromWorldTerrain()` -- preferred terrain helper
...
**IMPORTANT: Google 3D Tiles require TWO specific settings for Terms of Service compliance:**

1. Set `geocoder: IonGeocodeProviderType.GOOGLE` on the Viewer
2. Pass `onlyUsingWithGoogleGeocoder: true` to the factory helper

Both are **mandatory** -- omitting either violates Google's ToS even though the code still runs.
...
const tileset = await createGooglePhotorealistic3DTileset({
  onlyUsingWithGoogleGeocoder: true, // REQUIRED: ToS compliance acknowledgment
});
viewer.scene.primitives.add(tileset);
```
...
All widget toggles default to `true` (shown) except `vrButton` and `projectionPicker` which default to `false`.

To disable ALL widgets, set every one of these to `false`:

| Option | Default | Purpose |
|--------|---------|---------|
| `animation` | `true` | Playback speed controls (bottom-left) |
| `baseLayerPicker` | `true` | Imagery/terrain switcher (top-right) |
| `fullscreenButton` | `true` | Fullscreen toggle (bottom-right) |
| `geocoder` | `IonGeocodeProviderType.DEFAULT` | Search bar (top-right). Set to `false` to hide |
| `homeButton` | `true` | Reset camera to default view (top-right) |
| `infoBox` | `true` | Entity info popup (top-right) |
| `navigationHelpButton` | `true` | Mouse/touch help (top-right) |
| `sceneModePicker` | `true` | 2D/3D/Columbus toggle (top-right) |
| `selectionIndicator` | `true` | Green selection reticle on click |
| `timeline` | `true` | Time scrubber bar (bottom) |
| `projectionPicker` | `false` | Perspective/ortho toggle |
| `vrButton` | `false` | WebVR toggle |

### Disable All Widgets

When you need a completely clean viewer with no UI chrome (e.g., dashboard embedding), disable all 10 default-on widgets:
...
| `requestRenderMode` | `false` | Render only on changes (saves CPU/battery) |
| `maximumRenderTimeChange` | `0.0` | Max sim-time delta for auto-render (`Infinity` = only explicit renders) |
...
| Option | Default | Purpose |
|--------|---------|---------|
| `baseLayer` | `ImageryLayer.fromWorldImagery()` | Base imagery (`false` for none) |
...
| `globe` | `new Globe()` | `false` disables Earth entirely (space scenes) |
| `skyBox` | auto (WGS84) | `false` disables starfield |
| `skyAtmosphere` | auto (WGS84) | `false` disables atmospheric limb glow |

**DEPENDENCY TRAP -- `baseLayer` requires `baseLayerPicker: false`:**
When you provide a custom `baseLayer`, you **must** also set `baseLayerPicker: false`. Otherwise the picker widget tries to manage imagery layers itself and throws a runtime error. This applies any time you use a non-default base layer.

**CASCADING OPTIONS -- `globe: false` implies other disables:**
When you set `globe: false` (space scenes), you should also set `skyAtmosphere: false` (otherwise a floating blue ring appears where the atmosphere limb renders with no planet inside). You must also set `baseLayerPicker: false` since there are no imagery layers without a globe. Do NOT add terrain when there is no globe.
...
Returns a `Cesium3DTileset`. **Must `await` it**, then add to `viewer.scene.primitives`.

```js
import { createOsmBuildingsAsync, Cesium3DTileStyle } from "cesium";

// Basic usage -- always await and add to primitives
...
**Requires Google geocoder and ToS flag** (see Google Maps Platform section above).

```js
import { createGooglePhotorealistic3DTileset, IonGeocodeProviderType } from "cesium";

// Viewer MUST have geocoder: IonGeocodeProviderType.GOOGLE
...
A complete production setup: terrain, 3D buildings, camera positioned over a city, unnecessary widgets disabled.
...
Disable `globe`, `skyAtmosphere`, and `baseLayerPicker` together. Do NOT add terrain.
...
// skyBox (stars) remains visible -- appropriate for space
```

### Low-Power / Battery-Saving Mode

Use `requestRenderMode` to stop continuous 60fps rendering. The viewer only re-renders when the camera moves, data changes, or you call `scene.requestRender()`. Combine with `scene3DOnly` and `maximumRenderTimeChange: Infinity` for maximum savings.
...
// Entity API auto-triggers render requests -- no manual requestRender() needed for entities
// Call viewer.scene.requestRender() only after direct primitive/scene changes
```

### Custom Base Layer (e.g., OpenStreetMap)

**Must set `baseLayerPicker: false`** when providing a custom `baseLayer`. Otherwise CesiumJS throws a runtime error.

```js
import { Viewer, ImageryLayer, OpenStreetMapImageryProvider, SceneMode } from "cesium";

const viewer = new Viewer("cesiumContainer", {
  baseLayerPicker: false,  // REQUIRED with custom baseLayer
  baseLayer: new ImageryLayer(new OpenStreetMapImageryProvider({
    url: "https://tile.openstreetmap.org/",
  })),
  sceneMode: SceneMode.SCENE2D, // optional: flat 2D map
});
```
...
1. **Set `requestRenderMode: true`** for mostly-static apps. Reduces CPU/GPU and battery drain. Set `maximumRenderTimeChange: Infinity` for maximum savings. Call `scene.requestRender()` after direct scene changes (Entity API auto-triggers renders).
2. **Use `scene3DOnly: true`** when 2D/Columbus View is not needed. Saves GPU memory per geometry instance.
3. **Disable unused widgets** (`animation: false`, `timeline: false`, etc.) to reduce DOM overhead.
