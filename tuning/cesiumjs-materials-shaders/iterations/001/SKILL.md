...
> **Warning:** The GLSL function in custom Fabric `source` MUST be named `czm_getMaterial` and MUST return `czm_material`. Any other function name compiles without error but silently produces no material output.
...
When generating a Viewer for material demos, disable unnecessary widgets and position the camera to see the geometry.

```js
import { Viewer, Primitive, GeometryInstance, RectangleGeometry, Rectangle,
  MaterialAppearance, Material, Color, Cartesian2, Math as CesiumMath } from "cesium";
...
// Position camera to see the material on the rectangle
viewer.camera.setView({
  destination: Cesium.Cartesian3.fromDegrees(-95, 35, 800000),
...
> **Warning: MODIFY vs REPLACE semantics.** `MODIFY_MATERIAL` runs your shader BEFORE the lighting stage -- if you set `material.diffuse` to a constant color, PBR lighting still applies and may darken the result. For flat unlit color, use `REPLACE_MATERIAL` with `LightingModel.UNLIT`. Use `MODIFY_MATERIAL` only when you want to tweak existing material properties and keep PBR lighting.
...
Screen-space pipeline via `viewer.scene.postProcessStages` (`PostProcessStageCollection`). Stages execute in array order; each reads `colorTexture` and `depthTexture`.

> **Warning: Stage ordering matters.** Stages execute in the order they are added. For example, adding bloom AFTER a custom color-grading stage means bloom operates on the graded colors, not the original scene. If bloom should apply to raw scene output, add it before any color-grading stages.
...
> **Warning: GLSL version syntax.** CesiumJS post-process shaders use WebGL2 / GLSL ES 3.00 internally. Use `in` (not `varying`), `texture()` (not `texture2D()`), and `out_FragColor` (not `gl_FragColor`). The old WebGL1 syntax compiles without error on some drivers but produces a black screen or no effect on others.
