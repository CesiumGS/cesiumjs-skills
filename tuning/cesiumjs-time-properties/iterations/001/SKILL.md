...
> **Warning:** Always use `.clone()` when assigning a `JulianDate` to clock times (`startTime`, `stopTime`, `currentTime`). Without `.clone()`, you create a shared reference -- mutating the original date later silently changes the clock's time. Example: `clock.startTime = date` means any later `JulianDate.addSeconds(date, 60, date)` also shifts the clock's start time.
...
> **Warning:** `viewer.clock.shouldAnimate` defaults to `false`. Time-dynamic entities will appear frozen (stuck at `currentTime`) until you set `shouldAnimate = true`. This is the #1 reason time-dynamic visualizations appear broken. Either pass `shouldAnimate: true` to the Viewer constructor or set `viewer.clock.shouldAnimate = true` after creation.
...
viewer.clock.shouldAnimate = true;              // REQUIRED -- without this, time stands still
viewer.timeline.zoomTo(start, stop);            // MUST be called AFTER setting clock start/stop
...
> **Warning:** `viewer.timeline.zoomTo(start, stop)` must be called AFTER setting the clock's `startTime` and `stopTime`. If called before, the timeline widget displays the wrong time range. Always set clock bounds first, then call `zoomTo`.
...
> **Warning:** Default interpolation is `LinearApproximation` with degree 1, producing jagged piecewise-linear paths. For flight paths, orbits, or any curved trajectory, you MUST call `setInterpolationOptions` with `LagrangePolynomialApproximation` (degree 5 recommended). Without this, animations look broken -- positions jump between straight-line segments instead of following smooth curves.
...
> **Warning:** The second argument to `CallbackProperty(fn, isConstant)` controls caching. If `isConstant` is `true`, CesiumJS calls the function ONCE and caches the result forever -- subsequent frames reuse the stale value. Always pass `false` for time-varying values. This is the most subtle Property bug because the entity renders correctly on the first frame, then appears frozen.
