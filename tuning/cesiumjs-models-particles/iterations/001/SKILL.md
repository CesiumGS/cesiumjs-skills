...
### Scale vs minimumPixelSize

`scale` applies a fixed uniform multiplier. `minimumPixelSize` enforces a minimum on-screen pixel height. When both are set, **CesiumJS renders at whichever produces a LARGER on-screen size**. This means a distant model with `minimumPixelSize: 128` can appear disproportionately large. Use `maximumScale` to cap the enlargement.

| Goal | Use | Notes |
|---|---|---|
| Resize model uniformly | `scale` alone | Predictable at all distances |
| Prevent model from vanishing at distance | `minimumPixelSize` + `maximumScale` | Good for entities on a globe |
| Both | Set `maximumScale` to limit how large `minimumPixelSize` can grow the model |
...
After loading a model, position the camera so the model is visible. Use `camera.setView` for immediate positioning (not `flyTo`, which animates and may not finish before a screenshot).

```js
// Position camera to view the model from a reasonable distance
viewer.camera.setView({
  destination: Cartesian3.fromDegrees(-123.074, 44.050, 6000),
  orientation: {
...
> **Warning:** Accessing `model.boundingSphere`, `model.activeAnimations`, or `model.getNode()` before `readyEvent` fires can return `undefined` or throw. Always gate on `readyEvent` or check `model.ready === true` first.
...
> **Warning:** When using `HeightReference.CLAMP_TO_GROUND` (or any non-NONE value) with the Primitive API (`Model.fromGltfAsync`), you **must** pass `scene: viewer.scene`. Without the `scene` parameter, the height reference silently does nothing and the model floats at the ellipsoid.

```js
// Primitive API -- scene is REQUIRED for height reference to work
const model = await Model.fromGltfAsync({
  url: "truck.glb",
  heightReference: Cesium.HeightReference.CLAMP_TO_GROUND,
  scene: viewer.scene, // <-- without this, CLAMP_TO_GROUND silently fails
});

// Entity API -- scene is automatic
viewer.entities.add({
...
> **Warning:** A `ParticleSystem` with no `image` property renders **invisible particles**. Always provide an image. For demos and standalone examples, generate a canvas-based image (see "Canvas-Based Particle Images" below) to avoid external file dependencies.

### Smoke Trail

```js
import {
  Viewer, ParticleSystem, CircleEmitter, Color, Cartesian2,
  Transforms, Cartesian3, Math as CesiumMath
} from "cesium";

// Clean viewer for model/particle demos
...
const smokePosition = Cartesian3.fromDegrees(-75.157, 39.978, 0);

const smokeSystem = new ParticleSystem({
  image: createCircleImage(),  // canvas-based -- no external file needed
  ...
  modelMatrix: Transforms.eastNorthUpToFixedFrame(smokePosition),
  lifetime: 16.0,
  loop: true,
});
viewer.scene.primitives.add(smokeSystem);

// Position camera to see the particle effect
viewer.camera.setView({
  destination: Cartesian3.fromDegrees(-75.157, 39.976, 200),
  orientation: {
    heading: CesiumMath.toRadians(0),
...
const firework = new ParticleSystem({
  image: createCircleImage(),  // canvas-based -- see Canvas-Based section below
...
Generate particle textures dynamically instead of loading image files. This is the **recommended approach for demos and standalone examples** since it avoids external file dependencies.
...
const modelPosition = Cartesian3.fromDegrees(-123.074, 44.050, 5000);
...
// Position camera to see the model -- setView is immediate, no animation delay
viewer.camera.setView({
  destination: Cartesian3.fromDegrees(-123.074, 44.048, 5500),
  orientation: {
...
4. **Set `minimumPixelSize` carefully** -- large values force enlargement of distant models, increasing draw cost. Always pair with `maximumScale` to cap the effect.
