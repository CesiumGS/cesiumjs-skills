...
> **Warning:** Primitives are **immutable after first render** -- you cannot modify geometry positions, shape, or vertex data after the Primitive is added to the scene. To change geometry, remove the old Primitive with `scene.primitives.remove(primitive)` and create a new one. Per-instance attributes (color, show) **can** be updated at any time via `primitive.getGeometryInstanceAttributes(id)`.

> **Warning: vertexFormat mismatch causes silent failure.** If the geometry's `vertexFormat` does not match the Appearance, the geometry renders **black or invisible** with no error. Always use the appearance's static `VERTEX_FORMAT` constant (e.g., `PerInstanceColorAppearance.VERTEX_FORMAT`, `MaterialAppearance.MaterialSupport.TEXTURED.vertexFormat`).

## Primitive

When creating a viewer for geometry demos, disable unneeded widgets so the
geometry is the visual focus. Use `camera.setView` (not `flyTo`) to position
the camera immediately after adding primitives.
...
// Position camera to see the geometry immediately
viewer.camera.setView({
  destination: Cartesian3.fromDegrees(-100.0, 40.0, 1500000),
  orientation: {
    heading: 0.0,
...
> **Warning: GroundPrimitive requires real terrain.** `GroundPrimitive` drapes geometry onto terrain surfaces. With the default `EllipsoidTerrainProvider` (flat globe, no terrain data), ground primitives may render at the wrong height or be invisible. Always load a terrain provider (e.g., `Cesium.Terrain.fromWorldTerrain()`) when using `GroundPrimitive` or `GroundPolylinePrimitive`.

```js
import { GroundPrimitive, GeometryInstance, PolygonGeometry, PolygonHierarchy,
  ColorGeometryInstanceAttribute, ClassificationType, Cartesian3, Color, Terrain } from "cesium";

// Ensure terrain is loaded for ground primitives
const viewer = new Viewer("cesiumContainer", {
