...
**Always set `viewer.clock.shouldAnimate = true` before adding a particle system** -- particles only move when the clock is running.
...
### Framing Particle Effects

Use `viewer.camera.lookAt` with `HeadingPitchRange` for an oblique view that shows both the source marker and the rising plume:
...
viewer.camera.lookAt(
  ...
  new Cesium.HeadingPitchRange(
    ...
    Cesium.Math.toRadians(-20),  // pitch (negative = looking down)
    3000                         // range in meters
  )
);
```
