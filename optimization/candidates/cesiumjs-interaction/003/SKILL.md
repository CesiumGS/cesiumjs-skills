...
## Entity Selection via viewer.selectedEntity

Setting `viewer.selectedEntity` triggers the built-in InfoBox and
`viewer.trackedEntity` tracking. Always assign the `Entity` instance
(i.e., `picked.id` from a pick result), not the pick result itself.

```js
// Show InfoBox for a clicked entity
viewer.selectedEntity = picked.id;   // correct
// Clear selection
...
When an interaction demo needs polygon entities to pick, always construct
hierarchies with `new PolygonHierarchy(Cartesian3.fromDegreesArray([...]))`.
**`PolygonHierarchy` has no static helpers** -- `PolygonHierarchy.fromDegrees()`
and `PolygonHierarchy.fromEquatorialCoordinates()` do not exist and will throw
at runtime.
...
const hierarchy = new PolygonHierarchy(
  Cartesian3.fromDegreesArray([-80, 25, -75, 25, -75, 30, -80, 30])
);

// Wrong -- these static methods do NOT exist:
// PolygonHierarchy.fromDegrees(...)               // throws
// PolygonHierarchy.fromEquatorialCoordinates(...) // throws
```
