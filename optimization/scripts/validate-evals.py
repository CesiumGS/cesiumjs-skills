...
RESULTS_PATH = REPO_ROOT / "optimization" / "results" / "public-status.json"
...
def compute_scenario_hash(path: Path) -> str:
    """Compute a content hash for a scenario manifest.

    Uses SHA-256 over the normalized JSON representation to ensure
    deterministic hashing regardless of whitespace/formatting changes.
    """
    data = load_json(path)
    # Serialize with sorted keys and no whitespace for deterministic hash
    normalized = json.dumps(data, sort_keys=True, separators=(',', ':'))
    return hashlib.sha256(normalized.encode('utf-8')).hexdigest()


def validate_scenario(path: Path) -> tuple[str, str, str, str]:
    data = load_json(path)
    ...
    content_hash = compute_scenario_hash(path)
    return path.parent.name, data["id"], content_hash, data.get("runner_mode", "global-js")


def validate_results(skill_counts: dict[str, int], runner_mode_counts_by_skill: dict[str, dict[str, int]]) -> None:
    ...
    for entry in skills:
        ...
        expected_mode_counts = runner_mode_counts_by_skill.get(skill, {})
        normalized_actual = {mode: count for mode, count in runner_mode_counts.items() if count}
        normalized_expected = {mode: count for mode, count in expected_mode_counts.items() if count}
        if normalized_actual != normalized_expected:
            fail(
                f"{RESULTS_PATH}: {skill} runner_mode_counts {normalized_actual} "
                f"does not match manifests {normalized_expected}"
            )


def load_baselines() -> dict[str, dict[str, str]]:
    """Load existing baseline hashes if available."""
...
def validate_baselines(scenario_hashes: dict[str, dict[str, str]]) -> None:
    """Validate that committed scenario baselines match current manifests."""
    baselines = load_baselines()
    failures: list[str] = []

    for skill, scenarios in scenario_hashes.items():
        baseline_scenarios = baselines.get(skill, {})
        for scenario_id, current_hash in scenarios.items():
            baseline_hash = baseline_scenarios.get(scenario_id)
            if baseline_hash is None:
                failures.append(f"{skill}/{scenario_id}: missing baseline hash")
            elif baseline_hash != current_hash:
                failures.append(
                    f"{skill}/{scenario_id}: baseline hash is stale "
                    f"({baseline_hash[:12]}... != {current_hash[:12]}...)"
                )

    for skill, scenarios in baselines.items():
        if skill not in scenario_hashes:
            failures.append(f"{skill}: stale baseline skill with no scenarios")
            continue
        for scenario_id in scenarios:
            if scenario_id not in scenario_hashes[skill]:
                failures.append(f"{skill}/{scenario_id}: stale baseline for missing scenario")

    if failures:
        fail(
            "baseline hashes are out of date; run optimization/scripts/rebaseline-scenario.py for changed scenarios:\n  "
            + "\n  ".join(failures)
        )
...
def main() -> None:
    ...
    runner_mode_counts_by_skill: dict[str, dict[str, int]] = {}
    ...
    for path in scenario_paths:
        skill, scenario_id, content_hash, runner_mode = validate_scenario(path)
        key = (skill, scenario_id)
        if key in seen:
            fail(f"duplicate scenario id for {skill}: {scenario_id}")
        seen.add(key)
        skill_counts[skill] = skill_counts.get(skill, 0) + 1
        if skill not in runner_mode_counts_by_skill:
            runner_mode_counts_by_skill[skill] = {}
        runner_mode_counts_by_skill[skill][runner_mode] = (
            runner_mode_counts_by_skill[skill].get(runner_mode, 0) + 1
        )

        # Store hash by skill/scenario_id
        if skill not in scenario_hashes:
            scenario_hashes[skill] = {}
        scenario_hashes[skill][scenario_id] = content_hash

    validate_results(skill_counts, runner_mode_counts_by_skill)
    validate_baselines(scenario_hashes)

    print(
        f"[validate-evals] OK: {len(scenario_paths)} scenarios across "
        f"{len(skill_counts)} skills; baseline hashes match {BASELINES_PATH.relative_to(REPO_ROOT)}"
    )
