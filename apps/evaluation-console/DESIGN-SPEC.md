...
2. **Hand off:** An explicit rail affordance **"Hand off to Optimize →"** (or `⌘K`) `POST`s the focus set to the server, which writes `focus.json` in the **exact existing schema** (so the loop consumes it unchanged), stamping `source_run_id` to guard against a stale scorecard re-run.
3. **Gate:** Optimize's `o`/Start is **disabled** for any skill with no human-confirmed focus entry (rail greys it, hints "needs review"), with a clearly-labeled **"optimize without focus (exploratory)"** escape hatch for ad-hoc runs (judges' friction note).
